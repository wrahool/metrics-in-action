# this script produces Table A3 in the Appendix (check last digits)
...
model_tbl <- read_csv(glue("{agg_data_folder}/aggregated_data-60_periods-window-1-post-freq.csv"))
...
m4 <- felm(curr_topic_prop ~ log_eng_sig + last_topic_prop + log_avg_eng_sig + post_freq |
             media_id + window + final_topic | 0 | media_id + window,
           data = model_tbl)

m5 <- felm(curr_topic_prop ~ log_eng_sig + last_topic_prop + last_topic_prop_all + log_avg_eng_sig + post_freq|
             media_id + window + final_topic | 0 | media_id + window,
           data = model_tbl)
...
m6 <- felm(curr_topic_prop ~ log_eng_sig * topic_other + log_eng_sig * topic_entertainment + last_topic_prop + last_topic_prop_all + log_avg_eng_sig + post_freq |
             media_id + window + final_topic | 0 | media_id + window,
           data = model_tbl)

m7 <- felm(curr_topic_prop ~ log_eng_sig * topic_political + log_eng_sig * topic_entertainment + last_topic_prop + last_topic_prop_all + log_avg_eng_sig + post_freq |
             media_id + window + final_topic | 0 | media_id + window,
           data = model_tbl)
...
modelsummary(models = list(m2, m4, m5, m7),
             ...
             coef_omit = "^topic") %>%
  gtsave(filename = "results/tableA3.tex")
